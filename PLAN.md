...
**Status 2026-09-20:** documentation captured; architecture revised to match it (not FHIR; server-owned
virtual claim keyed by `consent_token`). Ready for Phase 0 once the remaining questions below are answered
...
### Phase 0 — Foundations  `[ ]`
- [ ] `pyproject.toml` (hatchling, `src/` layout, py ≥ 3.11, `py.typed`)
- [ ] Tooling: ruff, mypy `--strict`, pytest + pytest-asyncio + respx, pre-commit, coverage gate
- [ ] CI (GitHub Actions): lint → type-check → test matrix (3.11, 3.12, 3.13) → build → `twine check`
- [ ] Package skeleton per ARCHITECTURE.md §3 with empty `__init__.py` and the error hierarchy
- [ ] `SHASettings` (env-driven, fail-fast validation)
- [ ] `CHANGELOG.md`, `LICENSE`, `README.md` quick-start stub
- [ ] Remove placeholder `t.txt`

### Phase 1 — Domain  `[ ]`  *(pure Python, zero I/O, zero third-party imports)*
- [ ] Identifiers: `ConsentToken` (redacted repr), `PatientId`, `FacilityCode`, `PractitionerRef`, `ClaimGuid`, `LineGuid`, `FileId`
- [ ] Codes & enums: `Icd11Code`, `InterventionCode`, `SchemeCode`, `DocumentType`, `RegulationBody`, `ServiceType`,
      `CancelReason`, `DischargeReason`, `IdentificationType`, `NextOfKinIdType` (values from WORKFLOWS §8)
- [ ] `Money` (Decimal, KES) with quantisation policy
- [ ] Read models: `Eligibility`, `Authorization`, `VirtualClaim`, `Preauthorization` with derived behaviour only
- [ ] Status enums with `Unknown(raw)` fallback (`WorkflowState`, `AuthorizationStatus`, `PreauthStatus`)
- [ ] `RequestValidationPolicy` — SSoT for everything provably rejectable before an HTTP call
- [ ] Property-based tests (hypothesis) for VOs and Money; table tests for policy

### Phase 2 — Ports & Use Cases  `[ ]`
- [ ] Ports (`typing.Protocol`, split by consumer): `EligibilityGateway`, `ConsentGateway`, `VirtualClaimGateway`,
      `PreauthGateway`, `FileGateway`, `TokenProvider`, `Clock`
- [ ] Use cases: `VerifyEligibility`, `CaptureConsent`, `OpenVisit`, `Add/Remove{Intervention,Diagnosis,Line,Attachment}`,
      `PreviewClaim`, `SubmitClaim` (attempt-once + `SubmissionOutcomeUnknown`), `CloseClaim`, `DischargeInpatient`,
...
- [ ] Pydantic v2 schemas per resource group; alias generator for the `camelCase` (authorization/preauth) vs
...
- [ ] Mappers wire → domain snapshots; `error_translator.py` from `{error, message}` + status
- [ ] **Spec drift test**: every request spec's method+path+required fields asserted against `docs/api/spec/*.json`
- [ ] Contract tests against recorded UAT fixtures (sanitised)

### Phase 4 — Infrastructure  `[ ]`
- [ ] `HttpxTransport` (async; per-request timeout; correlation-ID header injection)
- [ ] `OAuth2ClientCredentials` (`POST /tenants/token`, form-urlencoded) with expiry-aware cache and single-flight refresh
- [ ] `RetryPolicy`: exponential backoff + jitter on idempotent calls (GETs, `/claims/preview`) and transient failures only;
      `submit` is attempted once (ARCHITECTURE §8)
- [ ] Structured logging at boundaries with redaction (bearer, consent_token, OTP, IDs, phones); optional `on_event` hook
- [ ] Contract tests with `respx`; `live` marker for sandbox smoke tests (off by default)
...
| Q7 | `identification_type` codes for `/patients/eligibility` | eligibility request model |
...
| Q10 | UAT `client_id`/`client_secret` for NaCare | `live` tests, fixture recording |
