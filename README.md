...
> **Status: `0.1.18`, pre-production.** All 49 published endpoints implemented. Eligibility, benefits,
> consent, the claim lifecycle, discharge and dispensing are verified live against DHA UAT. Two things are
> not ours to fix: `prescribe()` is blocked by an unpublished `patient_instruction` vocabulary (§9.8), and
> the pre-auth nested arrays are unverified (§14). Both are tracked in
> [docs/api/WORKFLOWS.md](docs/api/WORKFLOWS.md). See [PLAN.md](PLAN.md).

**This file is the whole guide** — every method, its parameters, the HTTP call it makes, the object it
returns, end-to-end recipes, and the gotchas that cost us days. Written for a developer who has never
touched SHA, the DHA API or this SDK: read §1–§4 once, then come back to the rest as reference.
...
6. [`sha.eligibility` — who is covered, for what](#6-shaeligibility) · [`sha.registries`](#64-sharegistries--who-is-this-person)
...
Beyond this file: [ARCHITECTURE](docs/ARCHITECTURE.md) (how the SDK is built and why),
[WORKFLOWS](docs/api/WORKFLOWS.md) (what UAT *actually* does, as opposed to what it documents),
[api/](docs/api/README.md) (reference generated from the official portal),
...
The client exposes seven **resources**:

| Attribute | What it's for | Section |
|---|---|---|
| `sha.auth` | Which facility am I? Is the credential working? | below |
| `sha.eligibility` | Is this person covered? For which benefits/interventions? Balances. | §6 |
| `sha.registries` | Turn an identity document into a CR number; can this member receive an OTP? | §6.4 |
...
**`sha.auth`** is plumbing you rarely touch — the SDK authenticates on its own — but three calls are
useful:

| Method | Returns |
|---|---|
| `identity()` | `Identity` — `facility`, `facility_id_type`, `tenant_id`, `tenant_name`, `issuer`, `client_id`, `issued_at`, `expires_at`, `seconds_remaining` *(prop)*. Read off the cached token; **never** returns the token itself. |
| `check()` | `True` if a token can be obtained, else raises. A readiness probe. |
| `token()` | `BearerToken` — `access_token`, `expires_in`, `token_type`. For code that must call the HIE directly (legacy paths, bots). Prefer the SDK's own methods: they never need it. |

```python
me = await sha.auth.identity()
print(me.facility, me.tenant_name)  # "which facility am I", without parsing a JWT yourself
```
...
### 6.4 `sha.registries` — who is this person?

`sha.eligibility` answers *what is this member entitled to*; the Client Registry answers *who are they*
and *can we reach them*. Worth calling **before** consent: SHA sends the visit OTP only to a confirmed,
active phone contact, and a member with none cannot consent by OTP at all — so this turns a failed
authorize into a question you asked first.

| Method | Sends | Returns |
|---|---|---|
| `find_patient(identification_number, identification_type=NATIONAL_ID)` | `GET /patients` | `PatientRecord \| None` — **`None`**, not an exception, when the registry holds nobody with that document |
| `contacts(patient)` | `GET /patients/contacts` | `tuple[PatientContact, …]` — empty means the OTP path is closed for this member |
| `can_consent_by_otp(patient)` | as above | `bool` — any confirmed, active phone on record |

**`PatientRecord`**: `patient_id` (`PatientId | None`), `first_name`, `middle_name`, `last_name`,
`full_name` *(prop)*, `gender`, `date_of_birth`, `identification_type`, `identification_number`, `phone`,
`citizenship`, `county`, `sub_county`, `ward`, `other_identifications`, `dependant_ids`.

**`PatientContact`**: `contact_id`, `value`, `contact_type`, `is_confirmed`, `is_active`, `is_main`,
`next_of_kin_full_name`, `can_receive_otp` *(prop)*.

```python
record = await sha.registries.find_patient("12345678", IdentificationType.NATIONAL_ID)
if record is None:
    return "not in the Client Registry"  # not an error — just nobody with that document
if not await sha.registries.can_consent_by_otp(record.patient_id):
    return "no confirmed phone on record — this member cannot consent by OTP"
```

> UAT answers a document it does not hold with `400 "zero results found in client registry"` rather than
> a 404. `find_patient` folds that into `None`, so a registry outage still raises and is never mistaken
...
session = await sha.emergency.open_case(
    ...
    ["SHA-19-001"],
    patient=None,
    notes="RTA, unconscious on arrival",  # notes are mandatory on SHA's side
...
with backoff; `submit` is attempted exactly once. It **stores nothing**: no database, no polling, no
background tasks — your `on_event` hook is where audit logging and metrics live. Layers follow Clean
Architecture and are enforced by `import-linter`. Details: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
